import { 
    ...
    SORT_ARRAY, GO_TO_LIST, 
    URL, 
    SAVE_COMBINATION        
} from './types';
import { asyncStorageFunc } from './generalActions';

export const selectProductToCalc = (selectedProducts, productId, subcategoryId) => dispatch => {
    ...
    if(!existInSelected || existInSelected.length == 0){
        return asyncStorageFunc().then((response) => {
            ...
            let products = data.allProducts.find(item => item.parentId == subcategoryId);
            let productsPaginated = products.data;
            const arrayLength = productsPaginated.length;
            let product;
            for(let i = 0; i < arrayLength; i++){
                product = productsPaginated[i].find(item => item.id === productId)
